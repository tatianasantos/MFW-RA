...
mom_ibnr_gen <- function(D, Pj, PId, PARAM, cmom){
  J = nrow(Pj)
  mom = rep(0,3)
  for(j in 1:J){
    mom[1] = mom[1] + A(D, Pj, PId, cmom)[j]*PARAM[[j,1]]/PARAM[[j,2]]
    mom[2] = mom[2] + B(D, Pj, PId, cmom)[j]*PARAM[[j,1]]/PARAM[[j,2]] + 
      (A(D, Pj, PId, cmom)[j])^2*PARAM[[j,1]]/(PARAM[[j,2]])^2
    mom[3] = mom[3] + C(D, Pj, PId, cmom)[j]*PARAM[[j,1]]/PARAM[[j,2]] +
      3*B(D, Pj, PId, cmom)[j]*A(D, Pj, PId, cmom)[j]*PARAM[[j,1]]/(PARAM[[j,2]])^2 + 
      2*(A(D, Pj, PId, cmom)[j])^3*PARAM[[j,1]]/(PARAM[[j,2]])^3
  }
  return(mom)
}
...
mom_cbni_gen <- function(D, PJ, PId, PARAM, cmom){
  J = length(PJ)
  mom = rep(0,3)
  for(j in 1:J){
    mom[1] = mom[1] + A_J(D, PJ, PId, cmom)[j]*PARAM[[1]]/PARAM[[2]]
    mom[2] = mom[2] + B_J(D, PJ, PId, cmom)[j]*PARAM[[1]]/PARAM[[2]] + 
      (A_J(D, PJ, PId, cmom)[j])^2*PARAM[[1]]/(PARAM[[2]])^2
    mom[3] = mom[3] + C_J(D, PJ, PId, cmom)[j]*PARAM[[1]]/PARAM[[2]] +
      3*B_J(D, PJ, PId, cmom)[j]*A_J(D, PJ, PId, cmom)[j]*PARAM[[1]]/(PARAM[[2]])^2 + 
      2*(A_J(D, PJ, PId, cmom)[j])^3*PARAM[[1]]/(PARAM[[2]])^3
  }
  return(mom)
}
